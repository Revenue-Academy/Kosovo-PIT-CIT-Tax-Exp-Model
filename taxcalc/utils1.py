...
def create_distribution_table(vdf, groupby, distribution_vardict, income_measure,
    ...
    if groupby == 'weighted_deciles':
        ...
        dist_table = pd.concat([dist_table, sum_row], ignore_index=True)
        
    del sum_row
    # ensure dist_table columns are in correct order
    #print(dist_table.columns.values.tolist())
    #print(distribution_vardict['DIST_TABLE_COLUMNS'])
...
def create_difference_table(vdf1, vdf2, groupby, tax_to_diff):
    ...
    if groupby == 'weighted_deciles':
        ...
        sum_row = pd.DataFrame(sum_row).transpose()
        diff_table = pd.concat([diff_table, sum_row], ignore_index=True)
    # delete intermediate Pandas DataFrame objects
    del gpdf
    del pdf
